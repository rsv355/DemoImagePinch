...
//                    mSessionData = new SessionData();
    ...
    public void PointsPayment(JSONObject object) {
        RequestParams p = new RequestParams();
        try {
        ...
        postFetch("/payment/postBackParam.do", p, new Task() {
            @Override
            public void onSuccess(JSONObject jsonObject) {
                try {
                    ...
                    eventBus.post(new CobbocEvent(CobbocEvent.PAYMENT_POINTS, true, jsonObject));
                } catch (JSONException e) {
                    eventBus.post(new CobbocEvent(CobbocEvent.PAYMENT_POINTS, false));
                }
            }
    ...
    * and wallet*/
    ...
    public void sendToPayU(JSONObject details, final String mode, final HashMap<String, Object> data,Double cashback1) throws JSONException
    {
        ...
        // first get the transaction keys
         RequestParams p = new RequestParams();
         Double Dis_amt=0.0;
        if(PaymentOptionsFragment.coupan_amt!=0)
        {
            p.put("couponUsed",PaymentOptionsFragment.choosedCoupan);
            Dis_amt=PaymentOptionsFragment.coupan_amt;
        }
        else if(PayUMoneyPointsFragment.coupan_amt!=0)
        {
            p.put("couponUsed",PayUMoneyPointsFragment.choosedCoupan);
            Dis_amt=PayUMoneyPointsFragment.coupan_amt;
        }
        else
        {
            if(!details.getString("cashbackAccumulated").toString().equals("null"))
            Dis_amt = details.getJSONObject("cashbackAccumulated").getDouble(Constants.AMOUNT);
            else
                Dis_amt=0.0;
        }
        ...
        p.put("revisedCashbackReceivedStatus", "0");
